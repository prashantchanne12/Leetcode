...
class MaximumPathSum:
    ...
    def find_maximum_path_sum_recursive(self, currentNode):
        ...
        # ignore the paths with negative sums, since we need to find the maximum sum we should ignore any path which has an overall negative sum
        maxPathSumFromLeft = max(maxPathSumFromLeft, 0)
        maxPathSumFromRight = max(maxPathSumFromRight, 0)

        # maximum path sum at the current node will be equal to the sum from the left subtree + the sum from the right subtree + val of current node
